...
- Produce **structured evidence** (latency, cost, refusals, faithfulness) that feeds `goni-router` thresholds and `config/council.yaml` seating.
...
6) **Surface** a filtered leaderboard by task tag; “promote” action writes a proposed routing rule (e.g., `coding -> codestral, deepseek; fallback llama4`). Ops can review/merge into `config/council.yaml`.
...
- **Router training data**: logs become supervised data for `goni-router` regret tests in `software/30-conformance.md` (label = chosen/best seat).
- **Data plane**: store logs as Arrow batches (schema lives beside other metrics in `software/50-data`); keep prompts hashed with minimal snippets for privacy.
...
- **Cloud (via Council)**: seats already in `config/council.yaml` (OpenRouter IDs or direct providers).
