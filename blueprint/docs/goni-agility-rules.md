...
| blueprint/prototype/** folders | Pin everything: exact model IDs, exact chunk sizes, exact seeds, exact eval datasets | These folders are allowed to be brutally specific because they are throw-away experiments |
| Everything else (/blueprint/software/kernel, blueprint/config/, old docker-compose, etc.) | Treated as scratch paper — can be changed or deleted without ceremony | If something graduates from prototype → reference design, it moves out with a dated decision |
...
- Put hard numbers in `blueprint/hardware/90-decisions.md`, `blueprint/software/90-decisions.md`, or `blueprint/prototype/**` with dates, seeds, and **valid until**.
- Leave `README.md`, `blueprint/docs/goni-story.md`, and `blueprint/docs/goni-whitepaper.md` inspirational and timeless; point to decision files for specifics.
