...
The OS + base image layer provides the **execution substrate** for a Goni node. It is not part of the kernel (𝒜, 𝒳, 𝒦, 𝓔), but defines:

- how goni-node / goni-http runs as a long-lived service,
- how CPU/GPU/NPU resources are exposed to the LLM runtime (𝓔),
- where persistent state for the Arrow Spine (𝒜) and models lives.
