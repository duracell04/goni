...
The **Vector Database** (VecDB) is the retrieval backend for the Context Plane (𝒳):

- Stores embeddings and metadata for chunks,
- Performs approximate nearest neighbour (ANN) search,
- Returns candidates as Arrow RecordBatches on the Arrow Spine (𝒜).
...
See `docs/specs/latent-state-contract.md` and
...
- **Arrow integration**
  - Expose results as RecordBatch with a fixed schema usable by 𝒳.
...
* **Arrow Spine invariant**
  search returns an Arrow RecordBatch wired into 𝒜; no JSON/serde in the hot path.
